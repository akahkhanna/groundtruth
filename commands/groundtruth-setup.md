...
6. **v2 claims contract** (experimental, **opt-in, off by default** — `GROUNDTRUTH_CONTRACT=1`). Instead of parsing the agent's prose, the agent ends each turn with one fenced `groundtruth-claims` block declaring exactly what it did; Groundtruth then checks that every claim exists in the diff (**CA** — claimed but absent) and every changed file is declared (**UC** — undeclared change). Detect it two ways: `printenv GROUNDTRUTH_CONTRACT`, and grep `CLAUDE.md` for a ` ```groundtruth-claims ` instruction.
   - It needs **both** halves: (a) the flag `env.GROUNDTRUTH_CONTRACT = "1"` in `.claude/settings.local.json` (same place as block), and (b) an instruction in `CLAUDE.md` telling the agent to emit the block. During soak it runs **alongside** v1 (both render on the card, so you can compare) and never blocks unless `GROUNDTRUTH_BLOCK` is *also* on and a CA fires.
   - If the user wants to try it → **offer to append the contract instruction to `CLAUDE.md`** (their file — show it first, write on yes):
     ````
