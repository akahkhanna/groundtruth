...
**Dogfood after the fixes** (reproduce with `node ../hooks/groundtruth.mjs --audit` from the repo root): self-match false positives in the engine source went to **0** (`Class 2`) and phantom-import FPs **3 → 0** (`Class 4`); self-checks **242 → 440**, red-team **14/14**.
...
The representative headline — a week of **in-workspace, post-0.9.4** sessions — remains the pending number. This first pass validated the harvester and the 0.9.3 integrity fix; it did not (and could not honestly) produce the rate.
...
### Completeness/honesty precision pass (2026-07, v0.9.5) — a session's own findings as a backlog

A long session spent **building and discussing Groundtruth itself** (a conversation *about* the auditor, spanning a second repo, with a verifier sub-agent running most turns) is the pathological worst-case for a text-matcher — and its own findings were a precision backlog. A per-class misfire audit of that session (~44 findings) put the three biggest classes through the same purge as Class-1: **read-intent asks** ("look at cluePrompts.js") no longer mint a hard, block-escalating open-loop (they demote to soft); **Groundtruth's own state files** discussed by bare basename no longer no-op-flag; and **`async_done`** — which cried wolf **23×/session (13/13 sampled FP)** and had **zero** frozen-corpus entries — now keys on a turn-verdict *stamp* rather than any "done/shipped" token in prose. Each Fable-verified (the async_done fix from a 13→1 transcript replay). Self-checks **428 → 440**. The **cross-workspace no-op** (a file edited in another repo, flagged absent from this diff) is stated as an unfixed limit. Details in [`../FIXES.md`](../FIXES.md) (v0.9.5).
