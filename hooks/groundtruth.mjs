 ...
 */
// A tool-file / non-repo path (proposed-rules.json, .claude/groundtruth/*, tmp|scratch, …) is GITIGNORED or
// ephemeral, so it can NEVER appear in a git diff — Class 3 (silent no-op) excludes it so a past-tense mention
...
export function analyze({ claim = '', diff = '', gitDiff = null, bashCmds = [], results = [], cwd = process.cwd(), bgPending = false, bashEvents = null, mutations = null }) {
  ...
  for (const m of claim.matchAll(C3_RE)) {
    ...
    // Exclude them via NONREPO_OR_TOOL (defined near analyze). (Corpus FP, session 79e00f4c.)
